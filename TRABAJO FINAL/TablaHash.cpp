...
{
private:
    unordered_map<int, uint64_t> indexMap; // Mapa en memoria para el índice
...
public:
    void loadIndex()
    {
        ...
        // Limpiar el mapa en caso de que ya tenga datos
        indexMap.clear();

        // Leer todo el archivo de índice y cargarlo en el mapa
        while (indexFile.read(reinterpret_cast<char *>(&dniInFile), sizeof(int)))
        {
            indexFile.read(reinterpret_cast<char *>(&pos), sizeof(uint64_t));
            indexMap[dniInFile] = pos;
        }
