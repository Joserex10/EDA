...
public:
    // Busca un dni en el hash_index y devuelve su posicion en el archivo de datos
    uint64_t findPositionInIndex(int dni)
    {
        ...
        // Buscar el DNI en el archivo de indice
        while (indexFile.read(reinterpret_cast<char *>(&dniInFile), sizeof(int)))
        {
            indexFile.read(reinterpret_cast<char *>(&pos), sizeof(uint64_t));
            if (dniInFile == dni)
            {
                indexFile.close();
                return pos; // DNI encontrado
            }
        }

        indexFile.close();
        return static_cast<uint64_t>(-1); // DNI no encontrado
    }
    ...
    void generateData()
    {
        ...
        string dataBuffer;
        vector<pair<int, uint64_t>> indexEntries;
        ...
        for (int i = 0; i < RECORD_COUNT; ++i)
        {
            ...
            uint64_t pos = static_cast<uint64_t>(dataFile.tellp()) + dataBuffer.size(); // Obtener la posicion actual mas el tamano del buffer
    ...
    void insertRecord()
    {
        ...
        indexFile.write(reinterpret_cast<const char *>(&dni), sizeof(int));
    ...
    void searchByDni(int dni)
    {
        ...
        dataFile.seekg(pos);
        string recordStr;
        getline(dataFile, recordStr); // Leer la linea completa del registro
    ...
    void deleteByDni(int dni)
    {
        ...
        // Moverse a la posicion del registro
        dataFile.seekg(pos);

        // Leer el primer caracter para verificar el estado
        char status;
        dataFile.get(status);
        if (status == 'D')
        {
        ...
        // Moverse al inicio del campo de estado y escribir 'D'
        dataFile.seekp(pos);
        dataFile.write("D", 1); // Marcar el registro como eliminado
...
int main()
{
    try
    {
        ...
        // Declaracion de ambos archivos solo una vez
        ifstream dataFileCheck(DATA_FILE, ios::binary);
        ifstream indexFileCheck(HASH_INDEX_FILE, ios::binary);

        // Verificar si ambos archivos estan disponibles y no estan vacios
        ...
        {
            cout << "Generando datos y archivo de indice hash..." << endl;
            db.generateData();
        }
        else
        {
        ...
        // Cerrar los archivos despues de la verificacion
        dataFileCheck.close();
