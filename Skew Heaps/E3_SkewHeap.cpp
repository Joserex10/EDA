...
// Inserta un nuevo valor en el heap
    ...
    // Funcion para extraer el minimo (raiz) del heap
    void extraerMin() {
        if (!raiz) {
            cout << "El heap esta vacio, no se puede extraer el minimo." << endl;
            return;
        }
        short minValor = raiz->valor;
        // Combina los subarboles izquierdo y derecho para obtener el nuevo heap
        raiz = combinar(raiz->izquierda, raiz->derecha);
        cout << "\nEl valor mininimo '" << minValor << "' ha sido extraido." << endl;
    }
...
void menuSkewHeap() {
    ...
    do{
        cout << "\n------------ MENU SKEW HEAP ------------\n\n";
        cout << "\t1. Insertar\n";
        cout << "\t2. Eliminar el minimo\n";
        cout << "\t3. Imprimir el heap\n";
        cout << "\t4. Salir\n\n";
        cout << "\tElige una opcion: "; cin >> opcion; validarDigito(opcion);
